...
import time
import signup.sms
import signup.db_interface
import signup.data_processing

#Helper to send text message
def sendMessage(phone, state, county):
    r = signup.data_processing.getday(state, county)
    if r is None:
    ...
    signup.sms.send_message(phone, to_send)
...
def update():
    try:
        ...
        with open("signup/live-us-counties.csv", "w", newline="") as csvfile:
        ...
        with open("signup/ago-us-counties.csv", "w", newline="") as csvfile2:
        ...
        signup.db_interface.call_on_all_phone_num(sendMessage)
        return True
